# -*- coding: utf-8 -*-
"""
Created on Sun Nov  8 16:14:48 2020
...
#S = N-E-I-J-D-H-R
dE/dt = (B(t)*(Chi * P + I+eps1*J)/N) * S - alpha1 * E
dP/dt = alpha1* E - alpha2 * P
dI/dt  = (delta*alpha2) * P - gamma * I
dJ/dt  = ((1-delta)*alpha2) * P - gammaA * J
dD/dt  = (q*phiD) * H
dH/dt  = (p*gamma) * I - (q*phiD) * H -((1-q)*phiR) * H
dR/dt  = gammaA * J + ((1-p)*gamma) * I + ((1-q)*phiR) * H 
dI_sum/dt  = (delta*alpha2) * E
dH_sum/dt  = (p*gamma) * I
...
class PhysicsInformedNN:
    ...
    def __init__(self, t_train, I_new_train, D_new_train, H_train, 
       ...
       # tf placeholders and graph
        ...
        self.S_pred, self.E_pred, self.PreS_pred, self.I_pred, self.J_pred, self.D_pred, self.H_pred, self.R_pred, self.I_sum_pred, self.D_sum_pred, self.H_sum_pred = self.net_u(self.t_u)  
        ...
        self.E_f, self.I_f, self.J_f, self.D_f, self.H_f, self.R_f, self.I_sum_f, self.H_sum_f, self.PreS_f = self.net_f(self.t_tf)
        ...
        self.lossF = tf.reduce_mean(tf.square(self.E_f)) + tf.reduce_mean(tf.square(self.I_f)) + \
            ...
            tf.reduce_mean(tf.square(self.PreS_f)) 
    ...
    def net_u(self, t):
        ...
        PreS = SEIJDHR[:,8:9]
        D_sum = D
        S = self.N-E-I-J-D-H-R-PreS 
        return S, E, PreS, I, J, D, H, R, I_sum, D_sum, H_sum
    ...
    def net_f(self, t):
        ...
        S, E, PreS, I, J, D, H, R, I_sum, D_sum, H_sum = self.net_u(t)  
        ...
        VCn = VCn*self.sf
        f_E = E_t - (betaI*(Chi * PreS + I + eps1 * J)/self.N) * S + alpha1 * E
        f_PreS = PreS_t - alpha1* E + alpha2 * PreS 
        f_I = I_t - (delta*alpha2) * PreS + gamma * I
        f_J = J_t - ((1-delta)*alpha2) * PreS + gammaA * J
        f_D = D_t - (q*phiD)*H
        ...
        return f_E, f_I, f_J, f_D, f_H, f_R, f_I_sum, f_H_sum, f_PreS  
    ...
    def predict(self, t_star):        
        ...
        return S, E, PreS, I, J, D, H, R, I_new, D_new, H_new, I_sum, D_sum, H_sum, BetaI, p, q 
...
#%%
    ...
    for j in range(5):
        ...
        # relative_path_results = '/Model2/Train-Results-'+casenumber+'/'
        ...
        # relative_path = '/Model2/Train-model-'+casenumber+'/'
