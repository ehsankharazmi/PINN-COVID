# -*- coding: utf-8 -*-
"""
Created on Sat Nov  7 22:18:06 2020
...
class PhysicsInformedNN:
    ...
    def __init__(self, t_train, I_new_train, D_new_train, H_train, 
       ...
       # tf placeholders and graph
        ...
        self.lossU = tf.reduce_mean(tf.square(self.I_sum_u - self.I_sum_pred)) + \
            40*tf.reduce_mean(tf.square(self.D_sum_u - self.D_sum_pred)) + \
            40*4*tf.reduce_mean(tf.square(self.H_u - self.H_pred)) + \
            40*2*tf.reduce_mean(tf.square(self.I_new_u[1:,:] - self.I_new_pred)) + \
            40*100*tf.reduce_mean(tf.square(self.D_new_u[1:,:] - self.D_new_pred))
    ...
    def callback(self, loss, lossU0, lossU, lossF, lossQ, d1, d2, d3):
        total_records_LBFGS.append(np.array([loss, lossU0, lossU, lossF, d1, d2, d3, lossQ]))
        print('L: %.3e, LU0: %.3e, LU: %.3e, LF: %.3e, LQ: %.3e, d1:%.2f, d2:%.2f, d3:%.2f'  
              % (loss, lossU0, lossU, lossF, lossQ, d1, d2, d3))        
    ...
    def train(self, nIter):  
        ...
        for it in range(nIter+1):
            ...
            if it % 100 == 0:
                ...
                total_records.append(np.array([it, loss_value, lossU0_value, lossU_value, lossF_value, d1_value, d2_value, d3_value, lossQ_value]))
                print('It: %d, L: %.3e, LU0: %.3e, LU: %.3e, LF: %.3e, LQ: %.3e, d1:%.2f, d2:%.2f, d3:%.2f, Time: %.2f' % 
                      (it, loss_value, lossU0_value, lossU_value, lossF_value, lossQ_value, d1_value, d2_value, d3_value, elapsed))
                start_time = time.time()
        ...
        if LBFGS:
            self.optimizer.minimize(self.sess,
                                    ...
                                    fetches = [self.loss, self.lossU0, self.lossU, self.lossF, \
                                               self.lossQ, self.d1, self.d2, self.d3,], 
...
#%%
    ...
    #%%   
        ...
        font = 24
        fig, ax = plt.subplots()  
        ax.plot(t_star, D_new_star/H_star*15, 'k--', marker = 'o',lw=2, markersize=5, label='Data-7davg')
